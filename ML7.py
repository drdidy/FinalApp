...
# SPX tables only (SPY fallback when needed)
# ═══════════════════════════════════════════════════════════════════════════════
...
def is_maintenance(dt: datetime) -> bool:
    """4–5 PM CT maintenance hour (closed)."""
    h = dt.hour
    return h == 16  # 16:00 <= t < 17:00

def in_weekend_gap(dt: datetime) -> bool:
    """
    Skip Fri 17:00 → Sun 17:00 CT.
    Treat any half-hour mark in this gap as 'do not count'.
    """
    wd = dt.weekday()  # Mon=0 ... Sun=6
    if wd == 5:  # Saturday
        return True
    if wd == 6 and dt.hour < 17:  # Sunday before 17:00
        return True
    if wd == 4 and dt.hour >= 17:  # Friday at/after 17:00
        return True
    return False
...
def count_effective_blocks(anchor_time: datetime, target_time: datetime) -> float:
    ...
    We count a block if the *end* of the half-hour is in open time.
    """
