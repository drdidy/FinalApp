...
def calculate_market_based_targets(anchor_price: float, market_data: pd.DataFrame) -> dict:
    """Calculate TP targets based on actual market volatility and range"""
    if market_data.empty:
        # Fallback: use minimal conservative targets
        daily_range = anchor_price * 0.005  # 0.5% fallback
    else:
        # Calculate actual daily range from market data
        if len(market_data) > 0:
            daily_high = market_data['High'].max()
            daily_low = market_data['Low'].min()
            daily_range = daily_high - daily_low
        else:
            daily_range = anchor_price * 0.005
    
    # Calculate realistic targets based on actual range
    tp1_distance = daily_range * 0.30  # 30% of actual daily range
    tp2_distance = daily_range * 0.50  # 50% of actual daily range
...
def calculate_dynamic_stop_buffer(anchor_price: float, market_data: pd.DataFrame) -> float:
    """Calculate stop buffer based on recent volatility"""
    if market_data.empty or len(market_data) < 2:
        return anchor_price * 0.003  # 0.3% fallback
    
    # Calculate recent volatility from price changes
    recent_returns = market_data['Close'].pct_change().dropna()
    if len(recent_returns) == 0:
        return anchor_price * 0.003
    
    volatility = recent_returns.std()
    # Use 1.5x volatility as stop buffer
    return anchor_price * max(0.002, min(0.01, volatility * 1.5))

def calculate_data_driven_probability(anchor_type: str, time_slot: str, market_data: pd.DataFrame) -> float:
    """Calculate probability based on actual market volatility and time patterns"""
    base_prob = 60.0  # Conservative base
    
    # Volatility adjustment based on actual data
    if not market_data.empty and len(market_data) >= 5:
        recent_vol = market_data['Close'].pct_change().tail(5).std()
        # Higher volatility = higher probability of anchor touches
        vol_adjustment = min(15, recent_vol * 1000)  # Scale to percentage
        base_prob += vol_adjustment
    
    # Time-based adjustment from actual volume patterns
    if not market_data.empty and 'Volume' in market_data.columns:
        hour = int(time_slot.split(':')[0])
        
        # Filter data to similar time periods if possible
        try:
            ...
            if not hour_data.empty:
                ...
                if total_avg_volume > 0:
                    ...
                    time_adjustment = (volume_ratio - 1) * 10
                    base_prob += time_adjustment
        except:
            pass  # Skip time adjustment if calculation fails
    
    return max(40, min(90, base_prob))

def calculate_entry_exit_table(projection_df: pd.DataFrame, anchor_type: str) -> pd.DataFrame:
    """Calculate entry/exit analysis based on actual market data"""
    ...
    market_data = st.session_state.get('es_anchor_data', pd.DataFrame())
    if market_data.empty:
        market_data = st.session_state.get('signal_data', pd.DataFrame())
    
    for idx, row in projection_df.iterrows():
        time_slot = row['Time']
        anchor_price = row['Price']
        
        # Calculate targets based on actual market data
        target_data = calculate_market_based_targets(anchor_price, market_data)
        tp1_distance = target_data['tp1_distance']
        tp2_distance = target_data['tp2_distance']
        
        # Dynamic stop based on actual volatility
        stop_buffer = calculate_dynamic_stop_buffer(anchor_price, market_data)
        
        # Calculate scenarios based on anchor type
        ...
        touch_prob = calculate_data_driven_probability(anchor_type, time_slot, market_data)
        ...
        analysis_rows.append({
            ...
            'Daily_Range': round(target_data['daily_range'], 2),
            'Touch_Probability': f"{touch_prob:.0f}%",
...
def calculate_anchor_entry_probability(anchor_type: str, time_slot: str) -> float:
    """Calculate entry probability based on market data"""
    # Get available market data
    market_data = st.session_state.get('es_anchor_data', pd.DataFrame())
    if market_data.empty:
        market_data = st.session_state.get('signal_data', pd.DataFrame())
    
    return calculate_data_driven_probability(anchor_type, time_slot, market_data)
...
# ═══════════════════════════════════════════════════════════════════════════════
# MARKET-DERIVED PROBABILITY FUNCTIONS
# ═══════════════════════════════════════════════════════════════════════════════

def calculate_volume_profile_strength(data: pd.DataFrame, price_level: float) -> float:
    """Calculate volume profile strength at price level using actual volume data"""
    if data.empty or 'Volume' not in data.columns:
        return 50.0
    
    # Find bars where price traded near the level
    price_tolerance = price_level * 0.005  # 0.5% tolerance
    nearby_bars = data[
        (data['Low'] <= price_level + price_tolerance) & 
        (data['High'] >= price_level - price_tolerance)
    ]
    ...
    level_volume = nearby_bars['Volume'].sum()
    total_volume = data['Volume'].sum()
    
    if total_volume == 0:
        return 50.0
    
    volume_concentration = (level_volume / total_volume) * 100
...
def detect_market_regime(data: pd.DataFrame) -> dict:
    """Detect market regime from actual price action"""
    if data.empty or len(data) < 20:
        return {'regime': 'INSUFFICIENT_DATA', 'trend': 'NEUTRAL', 'strength': 0, 'volatility': 1.5}
    
    # Calculate trend using price momentum
    closes = data['Close'].tail(20)
    price_change = (closes.iloc[-1] - closes.iloc[0]) / closes.iloc[0] * 100
    
    # Volatility from actual returns
    returns = closes.pct_change().dropna()
    volatility = returns.std() * np.sqrt(390) * 100  # Annualized intraday vol
...
def calculate_support_resistance_strength(data: pd.DataFrame, price_level: float) -> float:
    """Calculate support/resistance strength using actual pivot points"""
    ...
    tolerance = price_level * 0.008  # 0.8% tolerance
    
    high_touches = sum(1 for high in highs if abs(high - price_level) <= tolerance)
    low_touches = sum(1 for low in lows if abs(low - price_level) <= tolerance)
...
def calculate_time_edge_from_data(symbol: str, lookback_days: int = 30) -> dict:
    """Calculate time-of-day edge from actual historical performance"""
    try:
        # Fetch historical data
        end_date = datetime.now(CT_TZ).date()
        start_date = end_date - timedelta(days=lookback_days)
        historical_data = fetch_live_data(symbol, start_date, end_date)
        
        if historical_data.empty:
            return {}
        
        # Group by time of day and calculate performance metrics
        time_performance = {}
        
        for time_slot in ['08:30', '09:00', '09:30', '10:00', '10:30', '11:00', 
                         '11:30', '12:00', '12:30', '13:00', '13:30', '14:00', '14:30']:
            
            slot_data = historical_data.between_time(time_slot, time_slot)
            
            if slot_data.empty:
                continue
            
            # Calculate volatility and movement patterns
            ...
            if len(slot_returns) > 5:
                ...
                # Edge score based on volatility and directional bias
...
def get_market_hours_status() -> dict:
    """Get detailed market status with actual market hours"""
    current_time_ct = datetime.now(CT_TZ)
    ...
    return {
        'status': status,
        'session': session,
        'current_time': current_time_ct,
        'is_trading_day': is_weekday,
        'next_open': next_open
    }

# ═══════════════════════════════════════════════════════════════════════════════
# MAIN DASHBOARD AND SUMMARY
# ═══════════════════════════════════════════════════════════════════════════════
...
with summary_col1:
    ...
    spx_status = "Ready" if st.session_state.spx_analysis_ready else "Pending"
    stock_status = "Ready" if st.session_state.stock_analysis_ready else "Pending"
    signal_status = "Ready" if st.session_state.signal_ready else "Pending"
    contract_status = "Ready" if st.session_state.contract_ready else "Pending"
...
# ═══════════════════════════════════════════════════════════════════════════════
# QUICK ACTIONS PANEL WITH MARKET DATA
# ═══════════════════════════════════════════════════════════════════════════════
...
if any([
    ...
    with insight_tabs[0]:
        # Market regime analysis for active data
        if st.session_state.get('signal_ready', False):
    ...
    with insight_tabs[1]:
        ...
        if st.button("Calculate Time Edge", key="calc_time_edge"):
            with st.spinner("Analyzing time-of-day patterns..."):
                ...
                if time_edge_data:
                    edge_rows = []
                    for time_slot, metrics in time_edge_data.items():
                        edge_rows.append({
                            'Time': time_slot,
                            'Volatility': f"{metrics['volatility']:.2f}%",
                            'Avg Move': f"{metrics['avg_move']:.2f}%",
                            'Upward Bias': f"{metrics['upward_bias']:.1f}%",
                            'Edge Score': f"{metrics['edge_score']:.1f}"
                        })
    ...
    with insight_tabs[2]:
        # Volume analysis for active sessions
        if st.session_state.get('signal_ready', False):
            signal_data = st.session_state.get('signal_data', pd.DataFrame())
            if not signal_data.empty and 'Volume' in signal_data.columns:
                ...
                high_vol_bars = signal_data[signal_data['Volume'] > high_volume_threshold]
                
                vol_col1, vol_col2, vol_col3 = st.columns(3)
                with vol_col1:
                    st.metric("Average Volume", f"{avg_volume:,.0f}")
                with vol_col2:
                    st.metric("Volume Std Dev", f"{volume_std:,.0f}")
                with vol_col3:
                    st.metric("High Volume Bars", len(high_vol_bars))
                
                if not high_vol_bars.empty:
                    high_vol_avg_move = abs(high_vol_bars['Close'] - high_vol_bars['Open']).mean()
                    normal_vol_data = signal_data[signal_data['Volume'] <= high_volume_threshold]
                    normal_avg_move = abs(normal_vol_data['Close'] - normal_vol_data['Open']).mean() if not normal_vol_data.empty else 0
...
# ═══════════════════════════════════════════════════════════════════════════════
# FOOTER WITH MARKET DATA STATUS
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# APPLICATION COMPLETE - ALL MARKET DATA DRIVEN
# ═══════════════════════════════════════════════════════════════════════════════
